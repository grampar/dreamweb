...
<%@page import="nextep.userxl.Userxl, nextep.common.Common" %>
...
<%
	...
	List<Userxl> list = (List<Userxl>)request.getAttribute("boardList");
	...
	<tr>
    	<td>    		
    		<table border="0" cellspacing="0" cellpadding="0" width="92%">
                <tr>
                    <td><h3>수퍼크로스 유저 정보</h3></td>
                </tr>
            </table>     
            <table border="0" cellspacing="0" cellpadding="0" width="100%">
                <form name="myform" action="<%=ROOT_PATH %>/userxl/list.do" method="get" >
                    <tr>
                        <td width="80%"> 로그인 아이디:
            			...
            			<td width="20%">  
            			<input type="button" value="엑셀 다운로드" onclick="location.href='<%=ROOT_PATH %>/userxl/list_execl.do'" />    	                       
                        </td>
                    </tr>
                </form>
            </table>
			...
			<tr>
                    <td class="searchBG"><table border="0" cellspacing="1" cellpadding="1" width="100%" >
						...
						%>
                        <tr bgcolor="#ffffff">
                            <td class="rowAlignCt"><a href="<%=ROOT_PATH %>/userxl/view.do?user_no=<%=user_no%>&go_page=<%=go_page%>&search_str=<%=search_str%>"><%=login_id %></a></td>
